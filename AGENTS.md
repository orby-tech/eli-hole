...
- `lib/eli_hole/dns/handler.ex` — shared query pipeline for all transports (UDP/DoT/DoH): per-client rate-limit check → resolve + DNSSEC enforce + QueryLog; tags each query `:udp | :dot | :doh`. Throttled queries short-circuit to REFUSED (logged `:rate_limited`) before any upstream work; callers pass a bare source-IP `rate_key` (`nil` skips throttling)
- `lib/eli_hole/dns/rate_limiter.ex` — RateLimiter GenServer: per-client (source-IP) query throttling via atomic per-second ETS counters (`:ets.update_counter`, lock-free on the hot path); `enabled?`/`limit` config ETS-cached + persisted to `dns_settings` (PubSub `dns:rate_limit`); disabled by default, `allow?/1` is a no-op when off
...
- `lib/eli_hole/dns/query_log.ex` — query logging over two ETS stores: a capped live ring (`@max_recent` 1k, full entry maps) feeding `recent/1` + `recent_rate/0` + the `dns:queries` PubSub stream, and per-UTC-day aggregate counters (`{iso_date, kind, key}`, atomic `:ets.update_counter`) backing `stats/1`/`status_breakdown/1`/`dnssec_breakdown/1`/`top_domains/2`/`top_clients/2`. Daily totals are uncapped (no 10k scan); old days reaped via lexicographic-ISO `select_delete` (`@retention_days` 30). Stat funcs default to today and accept a `Date` **or a `Date.Range`** — a range sums the per-day counters across the window (bounded-ISO match-spec), powering the dashboard's Today/7-day/30-day long-term views; `daily_series/1` returns per-day `%{date, ok, blocked, error, rate_limited, total}` rows (zero-filled) for the daily-totals trend chart. A third store keys per-status counters by `{bucket_start_unix, status}` in 10-min slices over a rolling 24h window (`@series_buckets` 144), reaped with the daily prune; `series/0` returns the full zero-filled window (oldest first) for the dashboard "queries over time" chart. **Postgres-backed:** every logged entry is also mirrored to the `query_logs` table via `QueryHistory` (buffered + batch-flushed every 5s / 50 rows, best-effort so DNS never breaks); on boot all three ETS stores are rehydrated from Postgres (recent ring, daily aggregates, 24h series) so the log + stats survive a restart. DB rows pruned to the same 30-day retention; `persist:` config flag (off in test) gates the mirroring.
- `lib/eli_hole/dns/query_history.ex` — Postgres persistence layer behind `QueryLog`: `insert_many/1` (batch), `recent/1` + `recent_with_time/1` (rehydrate the ring), `daily_counts/2` + `series_counts/1` (rebuild ETS aggregates via SQL `GROUP BY`), `prune/1`, `clear/0`. `status`/`dnssec`/`transport` round-trip as strings, restored to fixed atom sets on read.
- `lib/eli_hole/dns/query_log_entry.ex` — QueryLogEntry Ecto schema (`query_logs` table; `queried_at` is the durable timestamp).
