defmodule EliHole.DNS.Whitelist do
  @moduledoc """
  Allowlist of domains that bypass the blocklist. A domain matched here is never
  blocked, even if a blocklist entry would otherwise match it. Mirrors
  `EliHole.DNS.Blocklist`: DB-backed entries cached in ETS for fast lookups.
  """
  use GenServer
  ...
  alias EliHole.DNS.WhitelistEntry

  require Logger

  @exact_table :whitelist_exact
  @wildcard_table :whitelist_wildcard
  @regex_table :whitelist_regex
  ...
  @doc "Check if a domain is whitelisted (bypasses blocklist). Fast path: ETS lookup."
  def allowed?(domain) when is_binary(domain) do
  ...
  @doc "List whitelist entries with pagination."
  ...
  def get_entry!(id), do: Repo.get!(WhitelistEntry, id)

  def create_entry(attrs) do
    result =
      %WhitelistEntry{}
      |> WhitelistEntry.changeset(attrs)
      |> Repo.insert()
  ...
  def update_entry(%WhitelistEntry{} = entry, attrs) do
    result =
      entry
      |> WhitelistEntry.changeset(attrs)
      |> Repo.update()
  ...
  def delete_entry(%WhitelistEntry{} = entry) do
  ...
  @doc "Return whitelist stats."
  def stats do
    total = Repo.one(from e in WhitelistEntry, select: count(e.id))
    enabled = Repo.one(from e in WhitelistEntry, select: count(e.id), where: e.enabled == true)
  ...
  defp broadcast_change do
    Phoenix.PubSub.broadcast(EliHole.PubSub, "dns:whitelist", :whitelist_changed)
  end
  ...
  defp load_entries_to_ets do
    ...
    Logger.info(
      "Whitelist loaded: #{:ets.info(@exact_table, :size)} exact, " <>
